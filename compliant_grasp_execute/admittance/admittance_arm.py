...
# The admittance math helpers (v3 / v4_2) live alongside this file in the
# self-contained ``admittance`` package; make this dir importable so the
# verbatim-copied controller modules keep working with their bare imports.
_ADMITTANCE_CONTROL_DIR = os.path.abspath(os.path.dirname(__file__))
...
class AdmittanceArm:
    ...
    def __init__(
        ...
        *,
        ...
        trans_lead_time: Optional[float] = None,
    ) -> None:
        ...
        # Published position carrot = x_current + v_cmd * trans_lead_time.
        # The default (None -> loop_period) matches the drag-to-teach / handover
        # behaviour where v_cmd is large enough that v*dt is a usable carrot. For
        # SLOW spring-driven motion (e.g. compliant grasp descent) v_cmd is small,
        # so v*dt is sub-millimetre and the QP OTG produces no motion; pass a
        # larger value (~0.1-0.15 s) so the carrot is big enough to track, exactly
        # like rot_lead_time does for rotation. Does NOT affect the admittance
        # dynamics (the integrator still uses real dt).
        self.trans_lead_time = (
            float(trans_lead_time) if trans_lead_time is not None else float(loop_period)
        )
    ...
    def _run(self) -> None:
        ...
        while not self._stop_evt.is_set():
            ...
            # Carrot = x_current + v_cmd * trans_lead_time (NOT v_cmd*dt): a sub-mm
            # dt-carrot is below the QP OTG resolution for slow spring-driven
            # motion, so the arm never moves. Projecting v_cmd over a longer lead
            # time gives a trackable carrot (same trick as rot_lead_time). The
            # admittance dynamics are unchanged (dx/v_cmd integrated with real dt).
            carrot = np.asarray(v_cmd, dtype=float) * self.trans_lead_time
            target_pose.position.x = x_current[0] + carrot[0]
            target_pose.position.y = x_current[1] + carrot[1]
            target_pose.position.z = x_current[2] + carrot[2]
