...
################################################################################################################################
    """"The following is the added funcitons"""

    def stop(self) -> bool:
        """Immediately stops gripper motion by settingGTO=0"""
        return self._set_var(self.GTO, 0)

    def _percent_to_255(self, value: int) -> int:
        """Convert 0-100% range to 0-255 gripper range"""
        return max(0, min(255, int(value * 2.55)))

    def _255_to_percent(self, value: int) -> int:
        """Convert 0-255 gripper range to 0-100% range"""
        return max(0, min(100, int(value / 2.55)))

    def move_percent(self, position_pct: int, speed_pct: int, force_pct: int) -> Tuple[bool, int]:
        """Move gripper using 0-100% values"""
        pos = self._percent_to_255(position_pct)
        spe = self._percent_to_255(speed_pct)
        for_val = self._percent_to_255(force_pct)
        return self.move(pos, spe, for_val)

    def move_and_wait_percent(self, position_pct: int, speed_pct: int, force_pct: int) -> Tuple[int, int]:
        """Move and wait using 0-100% values"""
        pos = self._percent_to_255(position_pct)
        spe = self._percent_to_255(speed_pct)
        for_val = self._percent_to_255(force_pct)
        final_pos, status = self.move_and_wait_for_pos(pos, spe, for_val)
        return self._255_to_percent(final_pos), status.value

    def get_current_position_percent(self) -> int:
        """Get current position in 0-100% range"""
        return self._255_to_percent(self.get_current_position())
################################################################################################################################

def main():
    # test open and closing the gripper
    gripper = RobotiqGripper()
    gripper.connect(hostname="172.168.0.32", port=63352)
    # gripper.activate()
