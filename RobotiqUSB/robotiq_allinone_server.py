    ...
    def _autoConnect(self):
        ...
        for port in ports:
            try:
                ...
                device = mm.Instrument(ser, self.slaveAddress, mm.MODE_RTU, close_port_after_each_call=False,
                                       debug=False)
...
class RobotiqGripperUSBServer(zerorpc.Server):
    def __init__(self, robot_port: str, slave_id: int):
        super().__init__()
        self.gripper = RobotiqGripper(robot_port, slave_id)
        self._last_val = 0
        self.gripper.resetActivate()
        print(f'current {self.get_gripper()}')

    def get_gripper(self):
        try:
            self._last_val = gripper_pos = self.gripper.getPosition()
        except mm.InvalidResponseError as error:
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        ...
        type=str,
        default="/dev/ttyUSB0",
        help="The port of the gripper, e.g., /dev/ttyUSB0",
    )
    arg_parser.add_argument(
        "--hand_slave",
        type=int,
        default="1",
        help="The slave id of the gripper, e.g., 1 or 9",
    )
    arg_parser.add_argument(
        "--hand_port",
        type=str,
    ...
    server = zerorpc.Server(RobotiqGripperUSBServer(args.hand_ip, args.hand_slave))
    server.bind(f"tcp://192.168.41.1:{args.hand_port}")  # Bind to a specific port
