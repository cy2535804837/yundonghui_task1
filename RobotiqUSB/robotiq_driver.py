...
class RobotiqGripper(mm.Instrument):
    ...
    def __init__(self, portname=AUTO_DETECTION, slaveAddress=9):
        """Create a RobotiqGripper object which can be used to control Robotiq grippers.

        Uses modbus RTU protocol USB/RS485 connection.

        Args:
            portname: The serial port name, for example\
                /dev/ttyUSB0 (Linux), /dev/tty.usbserial (OS X) or COM4\
                (Windows). It is necessary to allow permission to access this\
                connection using the bash command sudo chmod 666 /dev/ttyUSB0.\
                By default the portname is set to "auto". In this case the\
                connection is done with the first gripper found as connected\
                to the PC.
            slaveAddress: Address of the gripper (integer)\
                usually 9.
        """
        # Gripper slave address
        ...
        # Create the object using parent class constructor
        super().__init__(
            ser,
            self.slaveAddress,
            mm.MODE_RTU,
        ...
        # Attribute to monitor if the gripper is processing an action
        self.processing = False

        # Maximum allowed time to perform an action
        self.timeOut = 10

        # Dictionary where are stored description of each register state
        self.registerDic = {}
        self._buildRegisterDic()

        # Dictionary where are stored register values retrieved from the gripper
        self.paramDic = {}
        self.readAll()
    ...
    def _autoConnect(self):
        ...
        for port in ports:
            try:
                ...
                # Try to read the position request echo
                registers = device.read_registers(2000, 3, 4)
                posRequestEchoReg3 = registers[1] & 0b0000000011111111

                # Check if position request echo reflect the requested position
    ...
    def _buildRegisterDic(self):
        """Build a dictionary with comments to explain each register variable.

        Dictionary key are variable names. Dictionary value are dictionaries\
        with comments about each status of the variable (key=variable value,\
        value=comment)
        """
        ######################################################################
        ...
        gOBJdic[1] = (
            "Fingers have stopped due to a contact while opening before requested position. Object detected opening."
        )
        gOBJdic[2] = (
            "Fingers have stopped due to a contact while closing before requested position. Object detected closing."
        )
        gOBJdic[3] = "Fingers are at requested position. No object detected or object has been lost/dropped."
        ...
        gFLTdic[5] = (
            "Priority faults (LED is blue). Action delayed, activation (reactivation) must be completed "
        ...
        gFLTdic[10] = (
            "Major faults (LED blinking red/blue) - Reset is required "
            "(rising edge on activation bit rACT needed). Under minimum operating voltage."
        )
        gFLTdic[11] = (
            "Major faults (LED blinking red/blue) - Reset is required "
            "(rising edge on activation bit rACT needed). Automatic release in progress."
        )
        gFLTdic[12] = (
            "Major faults (LED blinking red/blue) - Reset is required "
            "(rising edge on activation bit rACT needed). Internal fault; contact support@robotiq.com."
        )
        gFLTdic[13] = (
            "Major faults (LED blinking red/blue) - Reset is required "
            "(rising edge on activation bit rACT needed). Activation fault, verify that no interference or "
        ...
        gFLTdic[15] = (
            "Major faults (LED blinking red/blue) - Reset is required "
            "(rising edge on activation bit rACT needed). Automatic release completed."
        )
        ...
        i = 0
        while i < 256:
            gPRdic[i] = f"Echo of the requested position for the Gripper: {i}/255"
            i += 1
        ...
        i = 0
        while i < 256:
            gPOdic[i] = f"Actual position of the Gripper obtained via the encoders: {i}/255"
            i += 1
        ...
        while i < 256:
            current = i * 10
            gCUdic[i] = f"The current is read instantaneously from the motor drive, approximate current: {current} mA"
            i += 1
    ...
    def readAll(self):
        ...
        """
        # Clear parameter dictionary data
        self.paramDic = {}
    ...
    def activate(self):
        ...
        """
        # Turn the variable which indicates that the gripper is processing
        # an action to True
    ...
    def resetActivate(self):
        """Reset the gripper (clear previous activation if any) and activate the gripper.

        During this operation the gripper will open and close.
        """
        # Reset the gripper
        self.reset()
    ...
    def goTo(self, position, speed=255, force=255):
        ...
        Args:
            position (int): Position of the gripper. Integer between 0 and 255. 0 being the open position and
                255 being the close position.
    ...
    def close(self, speed=255, force=255):
        ...
        This function closes the gripper by sending a target position, speed and force of 255.
        If high precision is required, please use goTo() function.

        Args:
            speed: Speed of the gripper closing movement (0-255).
    ...
    def open(self, speed=255, force=255):
        ...
        This function opens the gripper by sending a target position of 0 and the specified
        speed and force. If high precision is required, use goTo() function.

        Args:
            speed: Speed of the gripper opening movement (0-255).
            force: Force applied by the gripper (0-255).
        """
        self.goTo(0, speed, force)

    def goTomm(self, positionmm, speed=255, force=255):
        r"""Go to the requested opening expressed in mm.

        This function sends the gripper to a position expressed in mm.

        Note:
            * If positionmm is lower than the minimum position, positionmm is set to the minimum position.
            * If positionmm is higher than the maximum position, positionmm is set to the maximum position.

        Args:
            positionmm: Target position in millimeters.
            speed: Speed of the gripper movement (0-255).
            force: Force applied by the gripper (0-255).
            * The functions goToContact, close and open perform a reactivate if the gripper is not activated.
            * You must calibrate your gripper first in order to use this function.
        """
        ...
        if positionmm > self.openmm:
            raise Exception(f"The maximum opening is {self.openmm}")
...
################################################################################################################################
    ...
    def stop(self):
        """Immediately stop gripper motion by clearing the go-to bit (rGTO=0)."""
        self.readAll()
        ract = 1 if self.paramDic["gACT"] else 0
        command_reg = (ract << 8)  # rGTO=0, rACT preserved
        self.write_registers(1000, [command_reg, 0, 0])

    def _percent_to_255(self, value):
        """Convert 0-100% range to 0-255 gripper range."""
        return max(0, min(255, int(round(float(value) * 255.0 / 100.0))))

    def _255_to_percent(self, value):
        """Convert 0-255 gripper range to 0-100% range."""
        return max(0, min(100, int(round(float(value) * 100.0 / 255.0))))

    def goToPercent(self, position_pct, speed_pct=100, force_pct=100):
        """Go to position using percentages in [0, 100]."""
        position = self._percent_to_255(position_pct)
        speed = self._percent_to_255(speed_pct)
        force = self._percent_to_255(force_pct)
        self.goTo(position, speed, force)

    def goToPercentAndWait(self, position_pct, speed_pct=100, force_pct=100):
        """Go to position using percentages and wait for motion completion.

        Returns:
            tuple[int, int]: (final_position_percent, gOBJ_status_code)
        """
        position = self._percent_to_255(position_pct)
        speed = self._percent_to_255(speed_pct)
        force = self._percent_to_255(force_pct)
        ...
        motionTime = 0
        gobj = 0
        while motionTime < self.timeOut:
            motionTime = time.time() - motionStartTime
            self.readAll()
            gobj = self.paramDic["gOBJ"]
            # 0: moving, 1/2: stopped on object, 3: at requested position
            if gobj in (1, 2, 3):
                final_pos = self.paramDic["gPO"]
                return self._255_to_percent(final_pos), gobj
            time.sleep(0.005)

        raise Exception("Gripper motion did not complete before timeout.")

    def getPositionPercent(self):
        """Return the current position as percent in [0, 100]."""
        return self._255_to_percent(self.getPosition())

    # Compatibility aliases with robotiq_gripper.py naming
    def move_percent(self, position_pct, speed_pct=100, force_pct=100):
        """Alias of goToPercent(), matching socket-driver naming."""
        self.goToPercent(position_pct, speed_pct, force_pct)
        requested_pos = self._percent_to_255(position_pct)
        return True, requested_pos

    def move_and_wait_percent(self, position_pct, speed_pct=100, force_pct=100):
        """Alias of goToPercentAndWait(), matching socket-driver naming."""
        final_pos_pct, gobj_status = self.goToPercentAndWait(position_pct, speed_pct, force_pct)
        return final_pos_pct, gobj_status

    def get_current_position_percent(self):
        """Alias of getPositionPercent(), matching socket-driver naming."""
...
################################################################################################################################
    ...
    def _bitTomm(self, bit):
        r"""Convert a bit gripper opening in mm opening.
    ...
    def getPositionmm(self):
        r"""Return the position of the gripper in mm.
    ...
    def calibrate(self, closemm, openmm):
        ...
        Args:
            closemm: Distance between the fingers when the gripper is\
                fully closed.
            openmm: Distance between the fingers when the gripper is\
                fully open.
        """
    ...
    def isCalibrated(self):
        """Return if the gripper is calibrated.
