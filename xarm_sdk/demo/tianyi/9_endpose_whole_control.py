...
def main():
    ...
    xarm_manager.xarm_activate_controller(['endpose_wholebody_controller'])
    # xarm_manager.

    xarm_manager.get_logger().info("已激活endpose_wholebody_controller笛卡尔空间控制器，开始Topic控制...")
    # ========== 步骤4: 设置初始目标位姿 ==========
    # 创建左右臂的目标位姿对象
    target_pose_L = Pose()
    target_pose_R = Pose()
    ...
    target_pose_L.position.y = 0.598
    target_pose_L.position.z = 0.960
    # 设置左臂初始姿态（四元数，已归一化）
    target_pose_L.orientation.x = 0.5133696794509888
    target_pose_L.orientation.y = -0.5630228519439697
    target_pose_L.orientation.z = -0.33912
    target_pose_L.orientation.w = 0.56587

    # 设置右臂初始位置（单位：米）
    target_pose_R.position.x = 0.4
    target_pose_R.position.y = -0.589  # 负值表示在机器人右侧
    target_pose_R.position.z = 0.960
    # 设置右臂初始姿态（四元数，已归一化）
    target_pose_R.orientation.x = -0.39057
    target_pose_R.orientation.y = -0.4983730912208557
    target_pose_R.orientation.z = 0.43409
    target_pose_R.orientation.w = 0.64624
    ...
    while time.time() - start_time < 20.0:
        ...
        # ===== 左臂轨迹 =====
        target_pose_L.position.y = 0.598 + np.sin(current_time) * 0.1
        target_pose_L.position.z = 0.960 + np.cos(current_time) * 0.1
        
        # ===== 右臂轨迹 =====
        target_pose_R.position.y = -0.589 + np.sin(current_time) * 0.1
        target_pose_R.position.z = 0.960 + np.cos(current_time) * 0.1
        
        # ===== ✅ 双臂统一发布 =====
        topic_publisher.publish_endposetarget_dual(
            target_pose_l=target_pose_L,
            target_pose_r=target_pose_R,
            from_frame_l="waist_yaw_link",
            to_frame_l="left_tcp_link",
            from_frame_r="waist_yaw_link",
            to_frame_r="right_tcp_link",
            is_dual_arm_syn=False
        )
