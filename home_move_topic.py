...
def main():
    ...
    xarm_manager.hardware_arm_mode(3)  # highest stiffness — stiff position tracking like UR servoL
    ...
    action_call.jointspace_arm_L_controller([0.0, 1.18, 0.0, -1.3, 1.4, -0.13, 0.18])
    action_call.jointspace_arm_R_controller([0.0, -1.18, 0.0, -1.3, -1.4, -0.13, 0.18])


    
    xarm_manager.get_logger().info("移动机器人到home poses...")

    ## 2nd, 3rd, 5th, 7th joints are reversed for two arms
    # LEFT_ARM_HOME_JOINTS = [1.0, 0.3, 0.4, -2.3, -1.00, -0.20, 0.18]
    # RIGHT_ARM_HOME_JOINTS = [1.0, -0.3, -0.4, -2.3, 1.00, -0.20, -0.18]

    # LEFT_ARM_HOME_JOINTS = [
    #         0.7324762344360352,
    #         0.6585812568664551,
    #         0.24663496017456055,
    #         -2.487205982208252,
    ...
    # LEFT_ARM_HOME_JOINTS = [0.844, 0.026, -0.006, -2.216, -1.529, 0.039, 0.683]
    # RIGHT_ARM_HOME_JOINTS = [0.844, -0.026, 0.006, -2.216, 1.529, 0.039, -0.683]

    # LEFT_ARM_HOME_JOINTS = [   0.631,   0.800,  -0.667,  -0.798,  -1.021,  -0.026,   1.244]
    LEFT_ARM_HOME_JOINTS = [   0.833,   0.632,  -0.388,  -0.984,  -1.275,  -0.059,   1.125]
    RIGHT_ARM_HOME_JOINTS = [  1.389,  -0.680,  -0.109,  -2.020,   1.168,   0.070,  -0.779]


    action_call.jointspace_arm_L_controller(LEFT_ARM_HOME_JOINTS)
    action_call.jointspace_arm_R_controller(RIGHT_ARM_HOME_JOINTS)
    ...
    # action_call.jointspace_arm_R_controller([0.0, -1.18, 0.0, -1.3, 0.30, -0.20, 0.00])
