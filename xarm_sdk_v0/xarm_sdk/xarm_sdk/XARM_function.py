...
class ActionCall:
    ...
    @measure_time
    @require_robot_type(["tianyi2", "tianyi_max"])
    def jointspace_body_3joints_controller(self, target_positions: List[float], feedback_callback: Optional[Callable] = None):
        """
        控制身体前三个关节到指定的关节空间位置（3个关节）
        ...
        self.node.xarm_activate_controller('jointspace_body3joints_controller')
        ...
        return action_caller(
            node=self.node,
            action_name='/jointspace_body3joints_controller/jointspace',
            action_type=JointSpace,
            goal_msg=goal_msg,
            feedback_callback=feedback_callback,
        )
...
class TopicPublisher:
    ...
    def publish_jointspace_commands_body(self, target_positions: List[float]):
        ...
        """
        if len(target_positions) != 4 and len(target_positions) != 3:
            self.node.get_logger().error("target_positions长度必须为3或4，body")
            return None
        joint_commands = Float64MultiArray()
        joint_commands.data = target_positions
        topic_publisher(
