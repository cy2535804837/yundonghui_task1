...
class XARM_manager(Node):
    ...
    def xarm_right_arm_joint_efforts(self) -> Optional[List[float]]:
        ...
        if efforts is not None:
            ...
            self.get_logger().warn("未能读取到关节状态消息，无法提取关节力矩")
            return None
    ##====================TCP位姿获取, 获取base_frame到tcp_frame的位姿====================
    def get_tcp_pose(
        self,
        arm: str = "left",
        base_frame: str = "waist_yaw_link",
        tcp_frame: Optional[str] = None,
        timeout: float = 3.0,
    ) -> Optional[Dict[str, List[float]]]:
        """
        获取机械臂当前 TCP 位姿（通过 TF 查询）。

        Returns:
            dict: {"frame": base_frame, "child_frame": tcp_frame,
                   "translation": [x, y, z], "rotation": [qx, qy, qz, qw]}
            None: TF 查询失败/超时
        """
        arm = (arm or "").lower().strip()
        if tcp_frame is None:
            if arm in ("left", "l", "arm_l", "arm-left"):
                tcp_frame = "left_tcp_link"
            elif arm in ("right", "r", "arm_r", "arm-right"):
                tcp_frame = "right_tcp_link"
            else:
                self.get_logger().error(f"Unknown arm='{arm}', expected 'left' or 'right'")
                return None

        # tf2: lookup_transform(target, source) returns pose of `source` expressed in `target`
        result = lookup_tf_once(self, target_frame=base_frame, source_frame=tcp_frame, timeout=timeout)
        if result is None:
            return None
        ...
        return {
            ...
            "pose": [float(x) for x in translation + rotation],
        }
    ##===================================================================================
