...
def main():
    ...
    while time.time() - start_time < 20.0:
        ...
        target_pose_L.position.z = 0.1 + np.cos(current_time) * 0.1
        
        # 更新右臂位置（圆形轨迹，y方向反向）
        target_pose_R.position.y = -0.4 - np.sin(current_time) * 0.1
        target_pose_R.position.z = 0.1 + np.cos(current_time) * 0.1
        
        # 发布笛卡尔空间位置命令到Topic
        topic_publisher.publish_endposetarget_L(target_pose_L, from_frame = "waist_pitch_link")
        topic_publisher.publish_endposetarget_R(target_pose_R, from_frame = "waist_pitch_link")
